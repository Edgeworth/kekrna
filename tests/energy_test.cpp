...
TEST_F(EnergyTest, NNDBHairpinLoopExamples) {
  ...
  EXPECT_EQ(g_em->stack[C][A][U][G] + g_em->stack[A][C][G][U] + g_em->stack[C][A][U][G] +
      ...
      ComputeEnergy(kNNDBHairpin4, *g_em).energy);
...
TEST_F(EnergyTest, NNDBInternalLoopExamples) {
  EXPECT_EQ(g_em->stack[C][A][U][G] + g_em->stack[C][G][C][G] + g_em->InternalLoopInitiation(5) +
                std::min(g_em->internal_asym, NINIO_MAX_ASYM) +
                g_em->internal_2x3_mismatch[A][G][G][U] + g_em->internal_2x3_mismatch[G][G][A][C] +
...
TEST_F(EnergyTest, T04Tests) {
  ...
  EXPECT_EQ(
      45, ComputeEnergy(parsing::ParseDotBracketSecondary("GCAAAGCC", "((...).)"), *g_em).energy);
  EXPECT_EQ(
      57, ComputeEnergy(parsing::ParseDotBracketSecondary("CCCAAAAUG", ".(.(...))"), *g_em).energy);
  EXPECT_EQ(55, ComputeEnergy(parsing::ParseDotBracketSecondary("UACAGA", "(....)"), *g_em).energy);
  EXPECT_EQ(-6,
      ComputeEnergy(parsing::ParseDotBracketSecondary("AGGGUCAUCCG", ".(((...)))."), *g_em).energy);
  EXPECT_EQ(
      80, ComputeEnergy(parsing::ParseDotBracketSecondary("AGAGAAACAAAU", "(..(...)...)"), *g_em)
              .energy);
  EXPECT_EQ(95, ComputeEnergy(parsing::ParseDotBracketSecondary(
                    ...
                    *g_em)
                    .energy);
  EXPECT_EQ(77,
      ComputeEnergy(parsing::ParseDotBracketSecondary("CCCGAAACAG", "(..(...).)"), *g_em).energy);
  EXPECT_EQ(
      74, ComputeEnergy(
              parsing::ParseDotBracketSecondary("GACAGAAACGCUGAAUC", "((..(...)......))"), *g_em)
              .energy);
  ...
  EXPECT_EQ(179,
      ComputeEnergy(
          parsing::ParseDotBracketSecondary("UCUGAGUAAAUUGCUACGCG", "(....)((...).......)"), *g_em)
          .energy);

  // Special stacking - this is not implemented. TODO: Implement this?
  EXPECT_EQ(37,
      ComputeEnergy(parsing::ParseDotBracketSecondary("GGUCAAAGGUC", "((((...))))"), *g_em).energy);
  EXPECT_EQ(-45,
      ComputeEnergy(parsing::ParseDotBracketSecondary("GGGGAAACCCC", "((((...))))"), *g_em).energy);
  EXPECT_EQ(
      72, ComputeEnergy(parsing::ParseDotBracketSecondary("UGACAAAGGCGA", "(..(...)...)"), *g_em)
              .energy);
}
}
}
