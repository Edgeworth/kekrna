...
def run_subopt_benchmark(program, dataset, delta, num_file):
  nums = None
  if num_file:
    assert isinstance(program, KekRNA)  # Only kekrna supports num folding.
...
def parse_rna_from_args(parser, args):
  if args.benchmark:
    return None
  if bool(args.path) + bool(args.kekvault) + bool(args.cmd) != 1:
...
def process_command(*extra_args):
  ...
  parser.add_argument('-ks', '--kekrna-sorted', action='store_true')
  ...
  if args.kekrna:
    programs.append(KekRNA(args.kekrna_loc, False))
  if args.kekrna_sorted:
    programs.append(KekRNA(args.kekrna_loc, True))
