...
class Rnark:
  ...
  def efn(self, rna, logarithmic=False):
    ...
    benchmark_results, stdout = benchmark_command(
      fix_path('Eliotfn'), flag, rna.seq, rna.db())
    os.chdir(cwd)
    print(stdout)
...
def process_benchmark(programs, args):
  kekvault = KekVault('random')
  rnastructure = RNAstructure(fix_path('~/software/rna/rnastructure'))
  for program in programs:
    with open('%s_%s.results' % (program, kekvault.dataset), 'w') as f:
      idx = 1
      for rna in kekvault:
        results = []
...
def process_command(*extra_args):
  ...
  parser.add_argument('-k', '--kekvault')
  ...
  parser.add_argument('-pk', '--kekrna', action='store_true')
  ...
  try:
    ...
    kekrna_loc = default_paths.KEKRNA_PATH
  except ImportError:
  ...
  if args.kekrna_loc or args.kekrna:
    assert kekrna_loc
    programs.append(KekRNA(fix_path(kekrna_loc)))

  if args.benchmark:
    process_benchmark(programs, args)
  else:
    if bool(args.file) + bool(args.kekvault) + bool(len(args.cmd)) != 1:
      parser.error('Exactly one of --file or --kekvault or direct specification is required.')

    if args.file:
      rna = RNA.from_any_file(read_file(args.file))
    elif args.kekvault:
      kekvault = KekVault('archiveii')
      rna = kekvault[args.kekvault]
    elif args.cmd:
      if len(args.cmd) != 2:
