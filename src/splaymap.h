...
class SplayMap {
public:
  SplayMap() : ns(2), vals(2), root(DUMMY), size(0) {}
  ...
  bool Find(const Key& key) {
    if (root == DUMMY) return false;  // TODO remove this check?
    ...
    while (1) {
      ...
      if (key < n.k) {
        ...
        if (key < ns[l].k) {
          ...
          // Zag:
          ns[l].r = DUMMY;  // Split right child next level down.
        ...
        if (ns[r].k < key) {
          ...
          // Zag:
          ns[r].l = DUMMY;  // Split left child next level down.
  ...
  bool Delete(const Key& key) {
    if (Find(key)) {
      ...
      if (ns[root].r == DUMMY) {
        ...
        // will be DUMMY, so we can attach the left subtree there.
        int oldroot = root;
        root = ns[root].r;
        Find(key);
...
private:
  ...
  struct node_t {
    node_t() : k(), l(DUMMY), r(DUMMY) {}
    node_t(const Key& k_, int l_, int r_) : k(k_), l(l_), r(r_) {}
    Key k;
    int l, r;
  };
  std::vector<node_t> ns;  // TODO: add dummy at start, eliminate comparisons
