...
const char* FILENAMES[] = {
    "loop.dat", "stack.dat", "tstackh.dat", "tstacki.dat",
    "tloop.dat", "miscloop.dat", "dangle.dat", "int22.dat", "int21.dat", "coaxial.dat",
    "tstackcoax.dat", "coaxstack.dat", "tstack.dat", "tstackm.dat", "triloop.dat",
    "int11.dat", "hexaloop.dat", "tstacki23.dat", "tstacki1n.dat"
};
...
std::unique_ptr<datatable> LoadDatatable(const std::string& path) {
  ...
  opendat(paths[0].c_str(), paths[1].c_str(), paths[2].c_str(), paths[3].c_str(), paths[4].c_str(), paths[5].c_str(),
      paths[6].c_str(), paths[7].c_str(), paths[8].c_str(), paths[9].c_str(), paths[10].c_str(), paths[11].c_str(),
      paths[12].c_str(), paths[13].c_str(), paths[14].c_str(), paths[15].c_str(), paths[16].c_str(),
      paths[17].c_str(), paths[18].c_str(), dt.get());
  return dt;
}
...
computed_t Rnastructure::FoldAndDpTable(const primary_t& r, dp_state_t* dp_state) const {
  ...
  dynamic(structure.get(), data.get(), 1, 0, 0, nullptr,
      false, nullptr, TWOLOOP_MAX_SZ + 2, true, !use_lyngso, dp_state);
  return {{r, StructureToPairs(*structure)}, std::vector<Ctd>(r.size(), CTD_NA), energy_t(structure->GetEnergy(1))};
}
...
std::vector<computed_t> Rnastructure::Suboptimal(const primary_t& r, energy_t energy_delta) const {
  ...
  for (int i = 0; i < int(p_list.size()); ++i) {
    computeds.emplace_back(
        secondary_t(r, std::move(p_list[i])),
