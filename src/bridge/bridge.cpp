...
computed_t Kekrna::Fold(const primary_t& primary) const {
  return fold_fn(primary, nullptr);
}

computed_t Kekrna::FoldAndDpTable(const primary_t& primary, fold::fold_state_t* fold_state) const {
...
std::unique_ptr<RnaPackage> RnaPackageFromArgParse(const ArgParse& argparse) {
  ...
  if (argparse.HasFlag("r")) {
    ...
    return std::unique_ptr<RnaPackage>(new Kekrna(fold::FoldFunctionFromArgParse(argparse)));
  }
}
