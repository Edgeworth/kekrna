  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        const penergy_t base00 = gpt[st][piv][PT_P] * Boltzmann(gpc.augubranch[stb][pb]);
        const penergy_t base01 = gpt[st][piv - 1][PT_P] * Boltzmann(gpc.augubranch[stb][pl1b]);
        const penergy_t base10 = gpt[st + 1][piv][PT_P] * Boltzmann(gpc.augubranch[st1b][pb]);
        const penergy_t base11 = gpt[st + 1][piv - 1][PT_P] * Boltzmann(gpc.augubranch[st1b][pl1b]);
  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = 0; en < st; ++en) {
      ...
      for (int tpiv = st + 1; tpiv <= en + N; ++tpiv) {
        ...
        // baseAB indicates A bases left unpaired on the left, B bases left unpaired on the right.
        const penergy_t base00 = gpt[st][piv][PT_P] * Boltzmann(gpc.augubranch[stb][pb]);
        const penergy_t base01 = gpt[st][pl][PT_P] * Boltzmann(gpc.augubranch[stb][pl1b]);
        ...
        if (lspace) {
          const penergy_t base10 = gpt[st + 1][piv][PT_P] * Boltzmann(gpc.augubranch[st1b][pb]);
          const penergy_t base11 = gpt[st + 1][pl][PT_P] * Boltzmann(gpc.augubranch[st1b][pl1b]);
