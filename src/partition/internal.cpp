...
#define FILL_PENERGY(name) \
  static_assert(sizeof(name) / sizeof(*Decay(name)) == \
      sizeof(em.name) / sizeof(*Decay(em.name)), \
...
penergy_t FastHairpin(int st, int en) {
  ...
  penergy_t energy = energy::Boltzmann(
      energy::gem.HairpinInitiation(length) + energy::gem.AuGuPenalty(stb, enb));
...
penergy_t FastTwoLoop(int ost, int oen, int ist, int ien) {
  ...
  if (toplen == 1 && botlen == 2)
    return gppc.em.internal_1x2[gr[ost]][gr[ost + 1]][gr[ist]]
        [gr[ien]][gr[ien + 1]][gr[ien + 2]][gr[oen]];
  if (toplen == 2 && botlen == 1)
    return gppc.em.internal_1x2[gr[ien]][gr[ien + 1]][gr[oen]]
        [gr[ost]][gr[ost + 1]][gr[ost + 2]][gr[ist]];
  if (toplen == 2 && botlen == 2)
    return gppc.em.internal_2x2[gr[ost]][gr[ost + 1]][gr[ost + 2]]
        [gr[ist]][gr[ien]][gr[ien + 1]][gr[ien + 2]][gr[oen]];
  ...
  penergy_t energy = gppc.em.internal_init[toplen + botlen] * energy::Boltzmann(
