...
computed_t ParseCtdComputed(const std::string& prim_str, const std::string& pairs_str) {
  ...
  verify_expr(prim_str.size() == pairs_str.size(), "primary and pairs string need to be the same length");
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (c == 'P' || c == 'p') {
      ...
      verify_expr(prev >= 0 && p[prev] != -1 && (
          pairs_str[p[prev]] == 'n' || pairs_str[p[prev]] == 'N'), "invalid input");
      verify_expr(computed.base_ctds[i] == CTD_NA && computed.base_ctds[p[prev]] == CTD_NA, "invalid input");
    ...
    if (c == '3') {
      ...
      verify_expr(i - 1 >= 0 && p[i - 1] != -1 &&
          (computed.base_ctds[p[i - 1]] == CTD_NA ||
              computed.base_ctds[p[i - 1]] == CTD_UNUSED), "invalid input");
    ...
    if (c == 'P' || c == 'N' || c == ']') {
      ...
      if (s.top() >= 2) {
        if (computed.base_ctds[i] == CTD_NA)
          computed.base_ctds[i] = CTD_UNUSED;
        for (int j = p[i] + 1; j < i; ++j) {
          if (p[j] == -1) continue;
          if (computed.base_ctds[j] == CTD_NA)
            computed.base_ctds[j] = CTD_UNUSED;
          j = p[j];
        }
      }
      s.pop();
