...
Context::traceback_stack_t Context::ComputeExterior() {
  ...
  for (int st = N - 1; st >= 0; --st) {
    ...
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      // (   )3<   > 3'
      UPDATE_EXT(EXT, EXT, st, en - 1, base01 + em.dangle3[en1b][enb][stb]);
      // 5(   )<   > 5'
      UPDATE_EXT(EXT, EXT, st + 1, en, base10 + em.dangle5[enb][stb][st1b]);
      // .(   ).<   > Terminal mismatch
      UPDATE_EXT(EXT, EXT, st + 1, en - 1, base11 + em.terminal[en1b][enb][stb][st1b]);
      // .(   ).<(   ) > Left coax  x
      ...
      // (   ).<( * ). > Right coax backward
      if (st > 0)
        UPDATE_EXT(EXT_RCOAX, EXT, st, en - 1, base01 + em.MismatchCoaxial(en1b, enb, r[st - 1], stb));

      if (en < N - 1) {
        // (   )<(   ) > Flush coax
        auto enrb = r[en + 1];
        UPDATE_EXT(EXT, EXT_WC, st, en, base00 + em.stack[enb][enrb][enrb ^ 3][stb]);
        if (enrb == G || enrb == U)
          UPDATE_EXT(EXT, EXT_GU, st, en, base00 + em.stack[enb][enrb][enrb ^ 1][stb]);
      }
    }
  }
...
computed_t Context::Traceback(traceback_stack_t& q) {
