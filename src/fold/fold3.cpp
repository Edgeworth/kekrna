  ...
  for (int st = N - 1; st >= 0; --st) {
    ...
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      for (int l = 0; l <= max_inter; ++l) {
        ...
        lyngso[st][en][l] = std::min(lyngso[st][en][l], gem.InternalLoopAuGuPenalty(gr[st + l + 1], en1b) +
            gem.internal_other_mismatch[en1b][enb][gr[st + l]][gr[st + l + 1]] + val + gdp[st + l + 1][en - 1][DP_P]);
        lyngso[st][en][l] = std::min(lyngso[st][en][l], gem.InternalLoopAuGuPenalty(st1b, gr[en - l - 1]) +
            gem.internal_other_mismatch[gr[en - l - 1]][gr[en - l]][stb][st1b] + val + gdp[st + 1][en - l - 1][DP_P]);
      }

      // Update paired - only if can actually pair.
      if (ViableFoldingPair(st, en)) {
        // Stacking
        mins[DP_P] = std::min(mins[DP_P], gem.stack[stb][st1b][en1b][enb] + gdp[st + 1][en - 1][DP_P]);
        // Bulge
        ...
        for (int isz = 4; isz <= max_inter; ++isz) {
          auto val =
              base_internal_loop + gem.internal_init[isz] + std::min((isz - 2) * gem.internal_asym, NINIO_MAX_ASYM);
          mins[DP_P] = std::min(mins[DP_P], val +
              gem.InternalLoopAuGuPenalty(gr[st + isz], en2b) + gdp[st + isz][en - 2][DP_P]);
          mins[DP_P] = std::min(mins[DP_P], val +
              gem.InternalLoopAuGuPenalty(st2b, gr[en - isz]) + gdp[st + 2][en - isz][DP_P]);
        }
        ...
        mins[DP_P] = std::min(mins[DP_P],
            gem.internal_1x2[stb][st1b][st2b][gr[en - 3]][en2b][en1b][enb] + gdp[st + 2][en - 3][DP_P]);
        mins[DP_P] = std::min(mins[DP_P],
            gem.internal_1x2[en2b][en1b][enb][stb][st1b][st2b][gr[st + 3]] + gdp[st + 3][en - 2][DP_P]);
        mins[DP_P] = std::min(mins[DP_P],
            gem.internal_2x2[stb][st1b][st2b][gr[st + 3]][gr[en - 3]][en2b][en1b][enb] + gdp[st + 3][en - 3][DP_P]);
        ...
        mins[DP_P] = std::min(mins[DP_P], two_by_three + gem.InternalLoopAuGuPenalty(gr[st + 3], gr[en - 4]) +
            gem.internal_2x3_mismatch[gr[en - 4]][gr[en - 3]][st2b][gr[st + 3]] + gdp[st + 3][en - 4][DP_P]);
        mins[DP_P] = std::min(mins[DP_P], two_by_three + gem.InternalLoopAuGuPenalty(gr[st + 4], gr[en - 3]) +
            gem.internal_2x3_mismatch[gr[en - 3]][gr[en - 2]][gr[st + 3]][gr[st + 4]] + gdp[st + 4][en - 3][DP_P]);
        ...
        // (3<   ><   >) 3'
        mins[DP_P] = std::min(mins[DP_P], base_branch_cost + gdp[st + 2][en - 1][DP_U2] + gem.dangle3[stb][st1b][enb]);
        // (<   ><   >5) 5'
        mins[DP_P] = std::min(mins[DP_P], base_branch_cost + gdp[st + 1][en - 2][DP_U2] + gem.dangle5[stb][en1b][enb]);
        // (.<   ><   >.) Terminal mismatch
        mins[DP_P] = std::min(mins[DP_P],
        ...
        for (auto cand : cand_st[CAND_P_MISMATCH])
          mins[DP_P] = std::min(mins[DP_P], base_branch_cost + cand.energy + gdp[cand.idx + 1][en - 1][DP_U]);
        // (.(   )   .) Left outer coax
        ...
        for (auto cand : p_cand_en[CAND_EN_P_MISMATCH][en])
          mins[DP_P] = std::min(mins[DP_P], base_branch_cost + cand.energy + gdp[st + 1][cand.idx - 1][DP_U]);
        // (.   (   ).) Right outer coax
      ...
      for (auto cand : cand_st[CAND_U_LCOAX]) {
        const auto val = cand.energy + std::min(gdp[cand.idx + 1][en][DP_U_WC], gdp[cand.idx + 1][en][DP_U_GU]);
      ...
      for (auto cand : cand_st[CAND_U_WC])
        mins[DP_U_WC] = std::min(mins[DP_U_WC], cand.energy + std::min(gdp[cand.idx + 1][en][DP_U], 0));
      for (auto cand : cand_st[CAND_U_GU])
        mins[DP_U_GU] = std::min(mins[DP_U_GU], cand.energy + std::min(gdp[cand.idx + 1][en][DP_U], 0));
      for (auto cand : cand_st[CAND_U_RCOAX]) {
        // (   ).<( * ). > Right coax backward
        assert(st > 0);
        mins[DP_U_RCOAX] = std::min(mins[DP_U_RCOAX], cand.energy + std::min(gdp[cand.idx + 1][en][DP_U], 0));
      }
      ...
      // (   ). - 3' - U, U2
      const auto dangle3_base = gdp[st][en - 1][DP_P] + gpc.augubranch[stb][en1b] + gem.dangle3[en1b][enb][stb];
      if (dangle3_base < gdp[st][en][DP_U] && dangle3_base < cand_st_mins[CAND_U])
        cand_st_mins[CAND_U] = dangle3_base;
      // .(   ) - 5' - U, U2
      const auto dangle5_base = gdp[st + 1][en][DP_P] + gpc.augubranch[st1b][enb] + gem.dangle5[enb][stb][st1b];
      ...
      const auto terminal_base =
          gdp[st + 1][en - 1][DP_P] + gpc.augubranch[st1b][en1b] + gem.terminal[en1b][enb][stb][st1b];
      ...
      if (en + 1 < N) {
        ...
        const auto wc_flush_base = gdp[st][en][DP_P] + gpc.augubranch[stb][enb] + gem.stack[enb][enr1b][enr1b ^ 3][stb];
        const auto gu_flush_base = gdp[st][en][DP_P] + gpc.augubranch[stb][enb] + gem.stack[enb][enr1b][enr1b ^ 1][stb];
        if (wc_flush_base < CAP_E && wc_flush_base < gdp[st][en][DP_U])
          cand_st[CAND_U_WC_FLUSH].push_back({wc_flush_base, en});
        if (gu_flush_base < CAP_E && (enr1b == G || enr1b == U) && gu_flush_base < gdp[st][en][DP_U])
      ...
      // Paired cases
      // (.(   )   .) Left outer coax - P
      const auto plocoax_base = gdp[st + 2][en][DP_P] + gpc.augubranch[st2b][enb] + gpc.min_mismatch_coax;
      if (plocoax_base < gdp[st + 1][en][DP_U])
        cand_st[CAND_P_OUTER].push_back({plocoax_base, en});
      // (.   (   ).) Right outer coax
      const auto procoax_base = gdp[st][en - 2][DP_P] + gpc.augubranch[stb][en2b] + gpc.min_mismatch_coax;
      ...
      // ((   )   ) Left flush coax
      const auto plfcoax_base = gdp[st + 1][en][DP_P] + gpc.augubranch[st1b][enb] + gpc.min_flush_coax;
      if (plfcoax_base < gdp[st + 1][en][DP_U])
        cand_st[CAND_P_FLUSH].push_back({plfcoax_base, en});
      // (   (   )) Right flush coax
      const auto prfcoax_base = gdp[st][en - 1][DP_P] + gpc.augubranch[stb][en1b] + gpc.min_flush_coax;
