...
void InitFold(const primary_t& r) {
  ...
  for (base_t i = 0; i < 4; ++i)
    for (base_t j = 0; j < 4; ++j)
      g_augubranch[i][j] = g_multiloop_hack_b + AuGuPenalty(i, j);
...
energy_t FastTwoLoop(const primary_t& r, int ost, int oen, int ist, int ien) {
  ...
  if (toplen == 0 || botlen == 0)
    return Bulge(r, ost, oen, ist, ien);
  ...
  static_assert(TWOLOOP_MAX_SZ <= INITIATION_CACHE_SZ, "initiation cache not large enough");
  energy_t energy =
      g_internal_init[toplen + botlen] +
          std::min(std::abs(toplen - botlen) * g_internal_asym, NINIO_MAX_ASYM);

  energy += InternalLoopAuGuPenalty(r[ost], r[oen]);
...
energy_t FastHairpin(const primary_t& r, int st, int en, const std::vector<hairpin_precomp_t>& precomp) {
  ...
  energy_t energy = HairpinInitiation(length) + AuGuPenalty(stb, enb);
