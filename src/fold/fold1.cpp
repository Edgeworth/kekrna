  ...
  for (int st = N - 1; st >= 0; --st) {
    for (int en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      if (ViableFoldingPair(st, en)) {
        ...
        // (3<   ><   >) 3'
        p_min = std::min(p_min, base_branch_cost + gdp[st + 2][en - 1][DP_U2] + gem.dangle3[stb][st1b][enb]);
        // (<   ><   >5) 5'
        p_min = std::min(p_min, base_branch_cost + gdp[st + 1][en - 2][DP_U2] + gem.dangle5[stb][en1b][enb]);
        // (.<   ><   >.) Terminal mismatch
        p_min = std::min(p_min, base_branch_cost + gdp[st + 2][en - 2][DP_U2] + gem.terminal[stb][st1b][en1b][enb]);
        ...
        for (int piv = st + HAIRPIN_MIN_SZ + 2; piv < en - HAIRPIN_MIN_SZ - 2; ++piv) {
          ...
          p_min = std::min(p_min, base_branch_cost + gdp[st + 1][piv][DP_P] +
              gpc.augubranch[st1b][plb] + gdp[piv + 1][en - 1][DP_U] + gem.stack[stb][st1b][plb][enb]);
          // (   (   )) Right flush coax
          p_min = std::min(p_min, base_branch_cost + gdp[st + 1][piv][DP_U] +
              gpc.augubranch[prb][en1b] + gdp[piv + 1][en - 1][DP_P] + gem.stack[stb][prb][en1b][enb]);
        }
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        u2_min = std::min(u2_min, base11 + gem.terminal[pl1b][pb][stb][st1b] + gdp[piv + 1][en][DP_U]);
        // .(   ).<(   ) > Left coax - U
        ...
        u2_min = std::min(u2_min, val);
        if (st > 0)
          rcoax_min = std::min(rcoax_min, base01 + gem.MismatchCoaxial(
              pl1b, pb, gr[st - 1], stb) + right_unpaired);
