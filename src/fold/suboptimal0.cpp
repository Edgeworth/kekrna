...
//    }
    ...
    if (en == -1) {
      ...
      for (en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
        ...
        // (   ).<( * ). > Right coax backward
        if (st > 0 && a == EXT_RCOAX) {
          energy = base_energy + base01 + MismatchCoaxial(
        ...
        // .(   ).<(   ) > Left coax
        energy = base_energy + base11 + MismatchCoaxial(en1b, enb, stb, st1b);
    ...
    if (a == DP_P) {
      ...
      for (int ist = st + 1; ist < st + max_inter + 2; ++ist) {
        for (int ien = en - max_inter + ist - st - 2; ien < en; ++ien) {
          energy = base_energy + TwoLoop(r, st, en, ist, ien) + arr[ist][ien][DP_P];
          EXPAND1(energy, (ist, ien, DP_P));
        }
      }

      // Hairpin loop
      energy = base_energy + Hairpin(r, st, en);
      EXPAND0(energy);

      auto base_and_branch = base_energy + AuGuPenalty(stb, enb) + g_multiloop_hack_a + g_multiloop_hack_b;
      // (<   ><    >)
      ...
      for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
        ...
        if (a == DP_U_RCOAX) {
          if (st > 0) {
            energy = base_energy + base01 + MismatchCoaxial(pl1b, pb, r[st - 1], stb);
        ...
        // .(   ).<(   ) > Left coax - U, U2
        energy = base_energy + base11 + MismatchCoaxial(pl1b, pb, stb, st1b);
  ...
  for (const auto& struc : finished) {
    for (const auto& kek : struc.not_yet_expanded) {
      printf("%d %d %d\n", std::get<0>(kek), std::get<1>(kek), std::get<2>(kek));
    }
