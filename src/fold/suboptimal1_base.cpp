...
std::vector<expand_t> GenerateExpansions(const index_t& to_expand, energy_t delta) {
  ...
  if (en == -1) {
    ...
    for (en = st + HAIRPIN_MIN_SZ + 1; en < N; ++en) {
      ...
      if (st > 0 && a == EXT_RCOAX) {
        ...
        if (energy <= delta) exps.push_back({energy, {en + 1, -1, EXT}, {st, en - 1, DP_P}});
      }
      ...
      if (energy <= delta) exps.push_back({energy, {en + 1, -1, EXT}, {st, en - 1, DP_P}, {st, CTD_3_DANGLE}});

      // 5(   )<   > 5'
      energy = base10 + gem.dangle5[enb][stb][st1b] + gext[en + 1][EXT];
      if (energy <= delta) exps.push_back({energy, {en + 1, -1, EXT}, {st + 1, en, DP_P}, {st + 1, CTD_5_DANGLE}});
      ...
      if (en < N - 1) {
        ...
        if (energy <= delta) exps.push_back({energy, {en + 1, -1, EXT_RCOAX}, {st, en - 1, DP_P},
        ...
        if (energy <= delta) exps.push_back({energy, {en + 1, -1, EXT_WC}, {st, en, DP_P},
            {en + 1, CTD_FCOAX_WITH_PREV}, {st, CTD_FCOAX_WITH_NEXT}});

        if (enrb == G || enrb == U) {
          energy = base00 + gem.stack[enb][enrb][enrb ^ 1][stb] + gext[en + 1][EXT_GU];
          if (energy <= delta) exps.push_back({energy, {en + 1, -1, EXT_GU}, {st, en, DP_P},
              {en + 1, CTD_FCOAX_WITH_PREV}, {st, CTD_FCOAX_WITH_NEXT}});
        }
      }
    }
  ...
  if (a == DP_P) {
    ...
    for (int piv = st + HAIRPIN_MIN_SZ + 2; piv < en - HAIRPIN_MIN_SZ - 2; ++piv) {
      ...
      if (energy <= delta) exps.push_back({energy, {st + 2, piv, DP_P}, {piv + 1, en - 2, DP_U},
      ...
      if (energy <= delta) exps.push_back({energy, {st + 2, piv, DP_U}, {piv + 1, en - 2, DP_P},
      ...
      if (energy <= delta) exps.push_back({energy, {st + 2, piv - 1, DP_P}, {piv + 1, en - 1, DP_U},
      ...
      if (energy <= delta) exps.push_back({energy, {st + 1, piv, DP_U}, {piv + 2, en - 2, DP_P},
      ...
      if (energy <= delta) exps.push_back({energy, {st + 1, piv, DP_P}, {piv + 1, en - 1, DP_U},
      ...
      if (energy <= delta) exps.push_back({energy, {st + 1, piv, DP_U}, {piv + 1, en - 1, DP_P},
  ...
  for (int piv = st + HAIRPIN_MIN_SZ + 1; piv <= en; ++piv) {
    ...
    if (a == DP_U_RCOAX) {
      if (st > 0) {
        ...
        if (energy <= delta) exps.push_back({energy, {st, piv - 1, DP_P}});
    ...
    if (a == DP_U) {
      ...
      if (energy + gdp[piv + 1][en][DP_U]<= delta)
        exps.push_back({energy + gdp[piv + 1][en][DP_U], {st, piv, DP_P}, {piv + 1, en, DP_U}, {st, CTD_UNUSED}});
    }
    ...
    if (a == DP_U_WC || a == DP_U_GU) {
      ...
      if ((a == DP_U_WC && IsWatsonCrick(stb, pb)) || (a == DP_U_GU && IsGu(stb, pb))) {
        ...
        if (energy + gdp[piv + 1][en][DP_U] <= delta) exps.push_back({energy + gdp[piv + 1][en][DP_U], {st, piv, DP_P},
            {piv + 1, en, DP_U}});
      }
      continue;
    }
    ...
    if (a == DP_U && energy <= delta) exps.push_back({energy, {st, piv - 1, DP_P}, {st, CTD_3_DANGLE}});
    if (energy + gdp[piv + 1][en][DP_U] <= delta) exps.push_back({energy + gdp[piv + 1][en][DP_U],
        {st, piv - 1, DP_P}, {piv + 1, en, DP_U}, {st, CTD_3_DANGLE}});

    // 5(   )<   > 5' - U, U2
    energy = base10 + gem.dangle5[pb][stb][st1b];
    if (a == DP_U && energy <= delta) exps.push_back({energy, {st + 1, piv, DP_P}, {st + 1, CTD_5_DANGLE}});
    if (energy + gdp[piv + 1][en][DP_U] <= delta) exps.push_back({energy + gdp[piv + 1][en][DP_U],
    ...
    if (energy + gdp[piv + 1][en][DP_U] <= delta)
      exps.push_back({energy + gdp[piv + 1][en][DP_U],
          {st + 1, piv - 1, DP_P}, {piv + 1, en, DP_U}, {st + 1, CTD_MISMATCH}});
    ...
    if (energy <= delta) exps.push_back({energy, {st, piv - 1, DP_P}, {piv + 1, en, DP_U_RCOAX},
    ...
    if (piv < en) {
      ...
      if (energy <= delta) exps.push_back({energy, {st, piv, DP_P}, {piv + 1, en, DP_U_WC},
          {st, CTD_FCOAX_WITH_NEXT}, {piv + 1, CTD_FCOAX_WITH_PREV}});

      if (pr1b == G || pr1b == U) {
        energy = base00 + gem.stack[pb][pr1b][pr1b ^ 1][stb] + gdp[piv + 1][en][DP_U_GU];
        if (energy <= delta) exps.push_back({energy, {st, piv, DP_P}, {piv + 1, en, DP_U_GU},
            {st, CTD_FCOAX_WITH_NEXT}, {piv + 1, CTD_FCOAX_WITH_PREV}});
      }
    }
  }
