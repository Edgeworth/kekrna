...
void FuzzRna(const primary_t& primary, bool use_random_energy_model,
  ...
  for (const auto& kekrna : kekrnas) {
    ...
    auto computed = kekrna.FoldAndDpTable(primary, &state);
    // TODO check ctd descriptions are the same.
    kekrna_efns.push_back(energy::ComputeEnergy(secondary_t(computed)).energy);
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0;; ++i) {
    ...
    FuzzRna(primary, argparse.HasFlag("random"), kekrnas, rnastructure, eng);
  }
}
